...
async def test_display_gemini_processed_markdown_success(httpx_mock: HTTPXMock) -> None:
    ...
    httpx_mock.add_response(
        method="POST",
        url="http://127.0.0.1:8000/outputs/request",
    ...
    with patch("streamlit.empty", return_value=AsyncMock()) as mock_empty:
        ...
        placeholder.text = AsyncMock()
        await display_gemini_processed_markdown(filenames)
        placeholder.text.assert_called_once_with("*Test Output*")
