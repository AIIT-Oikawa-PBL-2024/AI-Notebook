...
async def test_get_file_by_id_not_found(session: AsyncSession) -> None:
    ...
    :return: None
    """
    retrieved_file = await files_cruds.get_file_by_id_and_user_id(session, 9999, "test_user_id")
...
async def test_delete_file_by_name(session: AsyncSession, test_user_id: str) -> None:
    ...
    await files_cruds.delete_file_by_name_and_userid(session, "test_file.pdf", test_user_id)
    file_id = int(file.id)
