...
with patch("app.utils.gemini_extract_text_from_audio.vertexai.init") as mock_vertexai_init:
    ...
    def test_extract_text_from_audio_success(mock_model_class: MagicMock) -> None:
        ...
        result = extract_text_from_audio("test_bucket", "test_audio.mp3")
        assert result == "test_text"

    @patch("app.utils.gemini_extract_text_from_audio.GenerativeModel")
    def test_extract_text_from_audio_unsupported_format(mock_model_class: MagicMock) -> None:
        with pytest.raises(
            InternalServerError, match="音声の文字起こしエラー: Unsupported audio file format."
        ):
            extract_text_from_audio("test_bucket", "test_audio.txt")

    @patch("app.utils.gemini_extract_text_from_audio.GenerativeModel")
    def test_extract_text_from_audio_internal_server_error(mock_model_class: MagicMock) -> None:
        mock_model_instance = MagicMock()
        mock_model_class.return_value = mock_model_instance
        ...
        with pytest.raises(InternalServerError, match="音声の文字起こしエラー: Test error"):
            extract_text_from_audio("test_bucket", "test_audio.mp3")
