...
async def test_get_file_by_id(session: AsyncSession) -> None:
    ...
    async with AsyncClient(
        ...
        print("Upload response text:", upload_response.text)  # デバッグ用出力
        assert upload_response.status_code == 200
        upload_data = upload_response.json()
        print("Upload data structure:", upload_data)  # ここでupload_dataの構造を確認
        ...
        print("Download response text:", response.text)  # デバッグ用出力
        assert response.status_code == 200
        data = response.json()
...
async def test_delete_file(session: AsyncSession) -> None:
    ...
    async with AsyncClient(
        ...
        print("Delete response text:", delete_response.text)  # デバッグ用出力
        assert delete_response.status_code == 200
        delete_data = delete_response.json()
        assert delete_data["detail"] == "ファイルが削除されました"
